...
OUTPUT_ROOT = r"C:\dev\GitHub\MIND\colab_data\drive\MyDrive\thesis_gnn_results\mind_graph_exps\tuning_stratified\cortex_mlp\shrink"

# Path for the aggregated summary output
SUMMARY_PATH = os.path.join(os.path.dirname(OUTPUT_ROOT), "shrink_aggregated_summary.csv")
...
def main():
    ...
    if "best_avg_test_f1_weighted" in df.columns:
        df = df.sort_values(by="best_avg_test_f1_weighted", ascending=False)
    elif "best_avg_test_loss" in df.columns:
